...
class SpikeSorter():
    ...
    def merge_spike_indices(self, spike_indices: List[np.ndarray], neuron_spikes: List[List[int]], tolerance: int=1) -> np.ndarray:
        """
    ...
    def get_waveforms(self, y: np.ndarray, spike_indices: np.ndarray, recenter: bool, window_shift_ratio: float=0.5) -> Tuple[List[np.ndarray], List[Dict]]:
        """
    ...
    def show_spike_train(self, grid, waveforms, waveform_info, figsize: Tuple[int]=(8, 8)):
        # determine the number of channels
        n_channels =  waveforms.shape[0]
